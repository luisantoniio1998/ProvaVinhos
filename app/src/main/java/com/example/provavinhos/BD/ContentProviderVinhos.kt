 ...
 * and Threads]({@docRoot}guide/topics/fundamentals/processes-and-threads.html#Threads).
    ...
    override fun query(
        ...
        val cursor = when (getUriMatcher().match(uri)) {
            URI_REGIOES -> TabelaBDVinhos(db).query(colunas, selection, argsSeleccao, null, null, sortOrder)
            ...
            URI_REGIAO_ESPECIFICA -> TabelaBDVinhos(db).query(colunas, "${BaseColumns._ID}=?", arrayOf("${id}"), null, null, null)
            else -> null
        }
